...
PHP_METHOD(Phalcon_Forms_Form, isValid) {
	...
	if (_5) {
		...
		ZEPHIR_CALL_METHOD(NULL, &validation, "__construct", NULL, 240);
		zephir_check_call_status();
	}
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_6), _7)
		{
			...
			if (Z_TYPE_P(&validators) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&validators), _9$$11)
				{
					...
					ZEPHIR_CALL_METHOD(NULL, &validation, "add", &_11, 241, &name, &validator);
					zephir_check_call_status();
				} ZEND_HASH_FOREACH_END();
			} else {
			...
			if (Z_TYPE_P(&filters) == IS_ARRAY) {
				ZEPHIR_CALL_METHOD(NULL, &validation, "setfilters", &_12, 242, &name, &filters);
				zephir_check_call_status();
			}
		} ZEND_HASH_FOREACH_END();
	} else {
	...
	ZEPHIR_INIT_NVAR(&element);
	ZEPHIR_CALL_METHOD(&messages, &validation, "validate", NULL, 243, data, entity);
	zephir_check_call_status();
