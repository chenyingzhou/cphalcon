...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getPhql) {
	...
	if (Z_TYPE_P(&columns) != IS_NULL) {
			...
			if (Z_TYPE_P(&columns) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&columns), _11$$25, _12$$25, _9$$25)
				{
					...
					if (Z_TYPE_P(&columnAlias) == IS_LONG) {
						...
						ZEPHIR_CALL_METHOD(&_13$$28, this_ptr, "autoescape", NULL, 481, &columnAlias);
						zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						if (Z_TYPE_P(&columnAlias) == IS_LONG) {
							...
							ZEPHIR_CALL_METHOD(&_15$$31, this_ptr, "autoescape", NULL, 481, &columnAlias);
							zephir_check_call_status();
		...
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			...
			if (Z_TYPE_P(&models) == IS_ARRAY) {
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _20$$34, _21$$34, _18$$34)
				{
					...
					if (Z_TYPE_P(&modelColumnAlias) == IS_LONG) {
						ZEPHIR_CALL_METHOD(&_22$$36, this_ptr, "autoescape", NULL, 481, &model);
						zephir_check_call_status();
						ZEPHIR_CONCAT_VS(&selectedColumn, &_22$$36, ".*");
					} else {
						ZEPHIR_CALL_METHOD(&_23$$37, this_ptr, "autoescape", NULL, 481, &modelColumnAlias);
						zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&selectedColumn);
						if (Z_TYPE_P(&modelColumnAlias) == IS_LONG) {
							ZEPHIR_CALL_METHOD(&_24$$39, this_ptr, "autoescape", NULL, 481, &model);
							zephir_check_call_status();
							ZEPHIR_CONCAT_VS(&selectedColumn, &_24$$39, ".*");
						} else {
							ZEPHIR_CALL_METHOD(&_25$$40, this_ptr, "autoescape", NULL, 481, &modelColumnAlias);
							zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(&_27$$41, this_ptr, "autoescape", NULL, 481, &models);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&models) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&models) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&models), _31$$42, _32$$42, _29$$42)
			{
				...
				if (Z_TYPE_P(&modelAlias) == IS_STRING) {
					ZEPHIR_CALL_METHOD(&_33$$44, this_ptr, "autoescape", NULL, 481, &model);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_34$$44, this_ptr, "autoescape", NULL, 481, &modelAlias);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					if (Z_TYPE_P(&modelAlias) == IS_STRING) {
						ZEPHIR_CALL_METHOD(&_35$$47, this_ptr, "autoescape", NULL, 481, &model);
						zephir_check_call_status();
						ZEPHIR_CALL_METHOD(&_36$$47, this_ptr, "autoescape", NULL, 481, &modelAlias);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&selectedModel);
		...
		ZEPHIR_CALL_METHOD(&_39$$49, this_ptr, "autoescape", NULL, 481, &models);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(&joins) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&joins), _41$$50)
			{
				...
				if (zephir_is_true(&joinType)) {
					ZEPHIR_CALL_METHOD(&_43$$52, this_ptr, "autoescape", NULL, 481, &joinModel);
					zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&_44$$52);
					ZEPHIR_CONCAT_SVSV(&_44$$52, " ", &joinType, " JOIN ", &_43$$52);
					zephir_concat_self(&phql, &_44$$52);
				} else {
					ZEPHIR_CALL_METHOD(&_45$$53, this_ptr, "autoescape", NULL, 481, &joinModel);
					zephir_check_call_status();
				...
				if (zephir_is_true(&joinAlias)) {
					ZEPHIR_CALL_METHOD(&_47$$54, this_ptr, "autoescape", NULL, 481, &joinAlias);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					zephir_array_fetch_long(&joinType, &join, 3, PH_NOISY, "phalcon/Mvc/Model/Query/Builder.zep", 807);
					if (zephir_is_true(&joinType)) {
						ZEPHIR_CALL_METHOD(&_50$$57, this_ptr, "autoescape", NULL, 481, &joinModel);
						zephir_check_call_status();
						ZEPHIR_INIT_NVAR(&_51$$57);
						ZEPHIR_CONCAT_SVSV(&_51$$57, " ", &joinType, " JOIN ", &_50$$57);
						zephir_concat_self(&phql, &_51$$57);
					} else {
						ZEPHIR_CALL_METHOD(&_52$$58, this_ptr, "autoescape", NULL, 481, &joinModel);
						zephir_check_call_status();
					...
					if (zephir_is_true(&joinAlias)) {
						ZEPHIR_CALL_METHOD(&_54$$59, this_ptr, "autoescape", NULL, 481, &joinAlias);
						zephir_check_call_status();
	...
	if (Z_TYPE_P(&group) != IS_NULL) {
		...
		if (Z_TYPE_P(&group) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&group), _58$$63)
			{
				...
				ZEPHIR_CALL_METHOD(&_60$$64, this_ptr, "autoescape", NULL, 481, &groupItem);
				zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_61$$65, this_ptr, "autoescape", NULL, 481, &groupItem);
					zephir_check_call_status();
			...
			if (Z_TYPE_P(&order) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&order), _65$$69)
				{
					...
					if (zephir_memnstr_str(&orderItem, SL(" "), "phalcon/Mvc/Model/Query/Builder.zep", 884) != 0) {
						...
						ZEPHIR_CALL_METHOD(&_67$$72, this_ptr, "autoescape", NULL, 481, &_68$$72);
						zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_71$$70, this_ptr, "autoescape", NULL, 481, &orderItem);
					zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						...
						if (zephir_memnstr_str(&orderItem, SL(" "), "phalcon/Mvc/Model/Query/Builder.zep", 884) != 0) {
							...
							ZEPHIR_CALL_METHOD(&_72$$75, this_ptr, "autoescape", NULL, 481, &_73$$75);
							zephir_check_call_status();
						...
						ZEPHIR_CALL_METHOD(&_76$$73, this_ptr, "autoescape", NULL, 481, &orderItem);
						zephir_check_call_status();
