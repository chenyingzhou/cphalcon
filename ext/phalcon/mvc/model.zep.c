	...
	if (Z_TYPE_P(&data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&data), _3, _4, _1)
		{
			...
			if (!(zephir_array_isset_fetch(&attribute, columnMap, &key, 0))) {
				...
				if (_5$$6) {
					ZEPHIR_CALL_FUNCTION(&reverseMap, "array_flip", &_6, 162, columnMap);
					zephir_check_call_status();
					ZEPHIR_OBS_NVAR(&attribute);
...
PHP_METHOD(Phalcon_Mvc_Model, query) {
	...
	ZEPHIR_CALL_METHOD(NULL, &criteria, "setmodelname", NULL, 436, &_3);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model, _preSave) {
	...
	if (ZEPHIR_GLOBAL(orm).virtual_foreign_keys) {
		ZEPHIR_CALL_METHOD(&_3$$8, this_ptr, "_checkforeignkeysrestrict", NULL, 439);
		zephir_check_call_status();
