...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, process) {
	...
	ZEPHIR_CALL_FUNCTION(&result, "openlog", NULL, 410, &name, &option, &facility);
	zephir_check_call_status();
	if (!zephir_is_true(&result)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "sprintf", NULL, 182, &_2$$3, &name, &facility);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 411, &_3$$3);
		zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&level, this_ptr, "logleveltosyslog", NULL, 412, &_4);
	zephir_check_call_status();
