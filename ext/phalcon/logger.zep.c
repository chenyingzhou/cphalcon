...
PHP_METHOD(Phalcon_Logger, log) {
	...
	ZEPHIR_CALL_METHOD(&intLevel, this_ptr, "getlevelnumber", NULL, 407, level);
	zephir_check_call_status();
	zephir_get_strval(&_0, message);
...
PHP_METHOD(Phalcon_Logger, addMessage) {
	...
	if (ZEPHIR_GE_LONG(&_0, level)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &item, "__construct", NULL, 408, &message, &levelName, &_1$$3, &_2$$3, &context);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Logger, getLevelNumber) {
	...
	if (Z_TYPE_P(level) == IS_STRING) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&levels, "array_flip", NULL, 162, &_0$$3);
		zephir_check_call_status();
