...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addComponent) {
	...
	if (_0) {
		...
		ZEPHIR_CALL_METHOD(NULL, &componentObject, "__construct", NULL, 132, componentValue);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_METHOD(&componentName, &componentObject, "getname", NULL, 133);
	zephir_check_call_status();
			...
			if (zephir_array_isset(&_11$$7, &roleInheritName)) {
				...
				if (Z_TYPE_P(&_13$$13) == IS_ARRAY) {
					ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_13$$13), _14$$13)
					{
						...
						ZEPHIR_CALL_FUNCTION(NULL, "array_push", &_16, 134, &checkRoleToInherits, &usedRoleToInherit);
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addRole) {
	...
	if (_0) {
		...
		ZEPHIR_CALL_METHOD(NULL, &roleObject, "__construct", NULL, 135, role);
		zephir_check_call_status();
	} else {
	...
	ZEPHIR_CALL_METHOD(&roleName, &roleObject, "getname", NULL, 136);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, allow) {
	...
	if (!ZEPHIR_IS_STRING(&roleName, "*")) {
		ZVAL_LONG(&_0$$3, 1);
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "allowordeny", NULL, 137, &roleName, &componentName, access, &_0$$3, func);
		zephir_check_call_status();
	} else {
		...
		if (Z_TYPE_P(&_1$$4) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_1$$4), _5$$4, _6$$4, _3$$4)
			{
				...
				ZVAL_COPY(&_2$$4, _3$$4);
				ZVAL_LONG(&_7$$5, 1);
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "allowordeny", NULL, 137, &innerRoleName, &componentName, access, &_7$$5, func);
				zephir_check_call_status();
			} ZEND_HASH_FOREACH_END();
		} else {
			...
			while (1) {
				...
				zephir_check_call_status();
					ZVAL_LONG(&_8$$6, 1);
					ZEPHIR_CALL_METHOD(NULL, this_ptr, "allowordeny", NULL, 137, &innerRoleName, &componentName, access, &_8$$6, func);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, deny) {
	...
	if (!ZEPHIR_IS_IDENTICAL(&_0, &roleName)) {
		ZVAL_LONG(&_1$$3, 0);
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "allowordeny", NULL, 137, &roleName, &componentName, access, &_1$$3, func);
		zephir_check_call_status();
	} else {
		...
		if (Z_TYPE_P(&_2$$4) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_2$$4), _6$$4, _7$$4, _4$$4)
			{
				...
				ZVAL_COPY(&_3$$4, _4$$4);
				ZVAL_LONG(&_8$$5, 0);
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "allowordeny", NULL, 137, &innerRoleName, &componentName, access, &_8$$5, func);
				zephir_check_call_status();
			} ZEND_HASH_FOREACH_END();
		} else {
			...
			while (1) {
				...
				zephir_check_call_status();
					ZVAL_LONG(&_9$$6, 0);
					ZEPHIR_CALL_METHOD(NULL, this_ptr, "allowordeny", NULL, 137, &innerRoleName, &componentName, access, &_9$$6, func);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, isAllowed) {
	...
	ZEPHIR_CALL_METHOD(&accessKey, this_ptr, "canaccess", NULL, 138, roleName, componentName, &access);
	zephir_check_call_status();
	...
	if (zephir_is_callable(&funcAccess)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &reflectionFunction, "__construct", NULL, 139, &funcAccess);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&reflectionParameters, &reflectionFunction, "getparameters", NULL, 140);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_METHOD(&numberOfRequiredParameters, &reflectionFunction, "getnumberofrequiredparameters", NULL, 141);
		zephir_check_call_status();
	...
	if (zephir_array_isset(&_0, &roleName)) {
		...
		if (Z_TYPE_P(&_2$$6) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_2$$6), _3$$6)
			{
				...
				ZEPHIR_MAKE_REF(&checkRoleToInherits);
				ZEPHIR_CALL_FUNCTION(NULL, "array_push", &_5, 134, &checkRoleToInherits, &usedRoleToInherit);
				ZEPHIR_UNREF(&checkRoleToInherits);
				zephir_check_call_status();
			} ZEND_HASH_FOREACH_END();
		} else {
