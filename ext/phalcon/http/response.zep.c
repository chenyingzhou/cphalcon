...
PHP_METHOD(Phalcon_Http_Response, redirect) {
	...
	if (externalRedirect) {
		...
		if (_0$$5) {
			...
			ZVAL_STRING(&_1$$5, "://");
			ZEPHIR_CALL_FUNCTION(&_2$$5, "strstr", NULL, 269, location, &_1$$5);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response, send) {
	...
	if (Z_TYPE_P(&content) != IS_NULL) {
		...
		if (_3$$5) {
			ZEPHIR_CALL_FUNCTION(NULL, "readfile", NULL, 354, &file);
			zephir_check_call_status();
		}
	}
	if (1) {
...
PHP_METHOD(Phalcon_Http_Response, setFileToSend) {
	...
	if (Z_TYPE_P(attachmentName) != IS_STRING) {
		ZEPHIR_CALL_CE_STATIC(&basePath, phalcon_helper_fs_ce, "basename", &_0, 355, &filePath);
		zephir_check_call_status();
	} else {
	...
	if (zephir_is_true(attachment)) {
		if ((zephir_function_exists_ex(ZEND_STRL("mb_detect_encoding")) == SUCCESS)) {
			ZEPHIR_CALL_FUNCTION(&_1$$6, "mb_detect_order", NULL, 356);
			zephir_check_call_status();
		...
		if (!ZEPHIR_IS_STRING(&basePathEncoding, "ASCII")) {
			ZEPHIR_CALL_FUNCTION(&_3$$7, "rawurlencode", NULL, 224, &basePath);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_FUNCTION(&_7$$8, "addcslashes", NULL, 190, &basePath, &_6$$8);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&currentHeadersRaw) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&currentHeadersRaw), _3, _4, _1)
		{
			...
			if (_5$$3) {
				...
				ZEPHIR_CALL_FUNCTION(&_7$$3, "strstr", &_8, 269, &key, &_6$$3);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_10$$5) {
					...
					ZEPHIR_CALL_FUNCTION(&_12$$5, "strstr", &_8, 269, &key, &_11$$5);
					zephir_check_call_status();
