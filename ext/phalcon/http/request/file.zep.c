...
PHP_METHOD(Phalcon_Http_Request_File, __construct) {
	...
	if (zephir_array_isset_string_fetch(&name, &file, SL("name"), 0)) {
		...
		if (zephir_is_true(&_1$$3)) {
			ZVAL_LONG(&_2$$4, 4);
			ZEPHIR_CALL_FUNCTION(&_3$$4, "pathinfo", NULL, 104, &name, &_2$$4);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, getRealType) {
	...
	ZVAL_LONG(&_0, 16);
	ZEPHIR_CALL_FUNCTION(&finfo, "finfo_open", NULL, 351, &_0);
	zephir_check_call_status();
	if (Z_TYPE_P(&finfo) != IS_RESOURCE) {
		RETURN_MM_STRING("");
	}
	zephir_read_property(&_0, this_ptr, SL("tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 352, &finfo, &_0);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(NULL, "finfo_close", NULL, 353, &finfo);
	zephir_check_call_status();
	RETURN_CCTOR(&mime);
...
PHP_METHOD(Phalcon_Http_Request_File, isUploadedFile) {
	...
	if (_0) {
		ZEPHIR_CALL_FUNCTION(&_1, "is_uploaded_file", NULL, 17, &tmp);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, moveTo) {
	...
	zephir_read_property(&_0, this_ptr, SL("tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_RETURN_CALL_FUNCTION("move_uploaded_file", NULL, 324, &_0, &destination);
	zephir_check_call_status();
	RETURN_MM();
