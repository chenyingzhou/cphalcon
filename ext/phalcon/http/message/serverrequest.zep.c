...
PHP_METHOD(Phalcon_Http_Message_ServerRequest, __construct) {
	...
	if (UNEXPECTED(ZEPHIR_IS_IDENTICAL(&_0, body))) {
		...
		ZEPHIR_CALL_METHOD(NULL, body, "__construct", NULL, 278);
	...
	ZEPHIR_CALL_METHOD(&_3, this_ptr, "processheaders", NULL, 280, headers);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, SL("headers"), &_3);
	ZEPHIR_CALL_METHOD(&_4, this_ptr, "processuri", NULL, 279, uri);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&files) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&files), _0)
		{
			...
			if (UNEXPECTED(Z_TYPE_P(&file) == IS_ARRAY)) {
				ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkuploadedfiles", &_2, 289, &file);
				zephir_check_call_status();
			} else {
