...
PHP_METHOD(Phalcon_Http_Cookie, delete) {
	...
	ZEPHIR_CALL_FUNCTION(&version, "phpversion", NULL, 275);
	zephir_check_call_status();
	if (zephir_start_with_str(&version, SL("7.2"))) {
		ZEPHIR_INIT_VAR(&_4$$5);
		zephir_time(&_4$$5);
		ZVAL_LONG(&_5$$5, (zephir_get_numberval(&_4$$5) - 691200));
		ZEPHIR_CALL_FUNCTION(NULL, "setcookie", NULL, 276, &name, &__$null, &_5$$5, &path, &domain, &secure, &httpOnly);
		zephir_check_call_status();
	} else {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "setcookie", NULL, 276, &name, &__$null, &options);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Http_Cookie, send) {
	...
	zephir_check_call_status();
	if (zephir_start_with_str(&version, SL("7.2"))) {
		ZEPHIR_CALL_FUNCTION(NULL, "setcookie", NULL, 276, &name, &encryptValue, &expire, &path, &domain, &secure, &httpOnly);
		zephir_check_call_status();
	} else {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "setcookie", NULL, 276, &name, &encryptValue, &options);
		zephir_check_call_status();
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_Cookie, assertSignKeyIsLongEnough) {
	...
	ZEPHIR_CALL_FUNCTION(&length, "mb_strlen", NULL, 277, &signKey);
	zephir_check_call_status();
	if (UNEXPECTED(ZEPHIR_LT_LONG(&length, 32))) {
		...
		ZEPHIR_CALL_FUNCTION(&_2$$3, "sprintf", NULL, 182, &_1$$3, &length);
		zephir_check_call_status();
